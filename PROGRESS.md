...
(#17, #38, #18, #19, #20), and Phase 4 (#21–#25) all merged. **#26 and #27
both merged to `main` (`5c3ef28`, `f43c609`), both closed by hand this session
— same stale-issue-left-open pattern as #17 / #20–#23 / #25 before them. #28
done, staged on `feat/edit-events-dateinput`** — see below. Next: #31 or #32
(both `ready`, depending only on #26/#27, both already merged).
...
**Phase 5 continues.** #31 (Notes section + row-level version check +
`ConflictDialog`) and #32 (Presence indicators on the edit view) are `ready`
— #31 depends only on #27 (merged), #32 only on #26 (merged). Decision 26 is
the version-check + `ConflictDialog` contract #31 needs — #27 and #28 already
built and proved out the version-check half (`updatePersonFields` /
`saveAdditionalNames` / `saveEvents`'s `WHERE … AND updated_at = $2` pattern
and its conflict-id reporting); #31 is the dialog UI on top of an
already-correct signal. #29 (Facts section) and #30 (Sources section) are not
yet `ready` — check their `Depends on:` once #31/#32 are under way.
...
Still pending across #14–#28: a deployed-function run (`supabase functions
...
Reference Numbers / Events sections) end to end. Do this as a dedicated
