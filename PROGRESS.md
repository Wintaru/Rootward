...
merged (`4c92f5e`), #22 merged (`00da1e6`), #23 merged (`5be36e5`) — all three
were still open on GitHub; #23 was closed by hand at the start of this session
(same as #17 / #20 / #21 / #22 before it), and #24 was labelled `ready`.
**#25 done, staged on `feat/person-profile`** — see below. Next: #24
(expand-in-place, `ready`) or start Phase 5 at #26 (Edit shell, `Depends on:
#10`, already unblocked but not yet labelled `ready` — a pre-existing gap, not
caused by #25). Nothing depends on #25 itself.
...
**Phase 4, issue #24 — Expand-in-place for collapsed branches** (`docs/SPEC.md`
§8.2, §10 item 24): on any card with relatives outside the current window, an
expand affordance that loads one more level for that branch only, without
re-centering — a scoped query, not a full refetch. Depends on #23 (merged), so
it is `ready` now. Alternatively, Phase 5 (edit view, #26–#37) can start at #26
— its only dependency (#10) has long been merged, so it is effectively
unblocked even though it was never labelled `ready` (a pre-existing gap this
session did not create or need to fix, since nothing in Phase 4 depends on it).

Nothing depends on #25 — it unblocks no other issue.
...
`/onboarding` → `/moderation` → `/tree/<root>` → `/person/<id>` end to end. Do
this as a dedicated integration pass, and restart the local stack first so the
...
`supabase db reset` is safe (all onboarding branches merged). #20–#25 add no
migration.
...
- #1–#23 and #38 are merged to `main`. #25 is on `feat/person-profile` (issue
  open until it merges). Later issues get `ready` as their dependencies close —
  do this when you finish an issue. #17 / #20 / #21 / #22 / #23 were each
  closed by hand after their work merged but the issue stayed open (`d6ee22f` /
  `536c920` / `4c92f5e` / `00da1e6` / `5be36e5`).
- Phase 1 complete (#4–#10). Phase 2 (GEDCOM) complete (#11–#16). Phase 3 (auth
  ...
  in progress — #21 / #22 / #23 merged, #25 staged; #24 remains.
