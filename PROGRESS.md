...
(#17, #38, #18, #19, #20), and Phase 4 (#21–#25) all merged. **#26 merged to
`main` (`5c3ef28`), closed by hand this session — same stale-issue pattern as
#17 / #20–#23 / #25 before it. #27 done, staged on
`feat/edit-name-gender-reference`** — see below. Next: #28 or #32 (both
`ready`, both depend only on #26); #31 is now also `ready` (depends only on
#27).
...
**Phase 5 is under way.** #28 (DateInput component + Events section) and #32
(Presence indicators on the edit view) are `ready` — each depends only on #26.
#31 (Notes section + row-level version check + `ConflictDialog`) is now also
`ready` — depends only on #27. `docs/SPEC.md` §8.3 / WAYFINDER decision 22 is
the `DateInput` contract #28 needs; decision 26 is the version-check +
`ConflictDialog` contract #31 needs — #27 already built and proved out the
version-check half (`updatePersonFields` / `saveAdditionalNames`'s
`WHERE … AND updated_at = $2` pattern); #31 is the dialog UI on top of an
already-correct signal.

Nothing depends on #26 or #27 being merged specifically — issues get labelled
`ready` on the strength of the dependency being _done_, ahead of the merge
itself (same bookkeeping-ahead-of-merge pattern as prior sessions).

Still pending across #14–#27: a deployed-function run (`supabase functions
...
Reference Numbers sections) end to end. Do this as a dedicated integration
pass, and restart the local stack first so the `config.toml` Google +
redirect-URL change loads.
...
- #1–#26 and #38 are merged to `main`. #27 is on
  `feat/edit-name-gender-reference` (issue open until it merges). Later issues
  get `ready` as their dependencies close — do this when you finish an issue.
  #17 / #20 / #21 / #22 / #23 / #24 / #25 / #26 / #38 were each closed by hand
  after their work merged but the issue stayed open (`d6ee22f` / `536c920` /
  `4c92f5e` / `00da1e6` / `5be36e5` / this session / `70c2c73` / `31e4bf3` /
  this session).
- Phase 1 complete (#4–#10). Phase 2 (GEDCOM) complete (#11–#16). Phase 3 (auth
  & onboarding) complete (#17, #38, #18, #19, #20). Phase 4 (tree view,
  #21–#25) complete. Phase 5 (edit view, #26–#37) under way — #26 merged, #27
  staged, the rest not started.
