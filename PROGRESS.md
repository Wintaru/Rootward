...
**Phase:** 3 — Auth & onboarding is **complete**. Phase 0 (#1–#3), Phase 1
(#4–#10), Phase 2 (#11–#16) all merged. #17 merged (`d6ee22f`). **#38 (Phase 8
seed data) pulled forward** — it blocks #18's `pg_trgm` threshold tuning and
#21's pedigree-collapse work, so it was built out of phase order. #38 merged
(`31e4bf3`). #18 merged (`3bb3215`). #19 merged (`fd4b7f8` + `933cc9e`).
#20 done, staged on `feat/invite-flow` — see below. **Issue #17 was still open
on GitHub though its work merged in `d6ee22f`; closed it this session.** Next is
Phase 4 (the tree view, #21–#25).
...
**Phase 4, issue #21 — `family-chart` integration + custom `PersonCard`**
(`docs/SPEC.md` §8.2, §10 item 21): the `family-chart` v2 hourglass view with
custom HTML cards (photo / gender-tint silhouette, name, birth–death years),
focus ring, blue/orange gender tint. Then #22–#25 (generation bands,
`getNeighborhood` wiring + deep links, expand-in-place, the read-only profile).
`#38`'s seed tree (28-person Ashby family, pedigree collapse) is the fixture.

Still pending across #14–#20: a deployed-function run (`supabase functions
serve`) plus a real signed-in browser session driving `/login` → `/import` →
`/onboarding` → `/moderation` end to end. Do this as a dedicated integration
...
`feat/onboarding-match` and `feat/onboarding-ui` are now merged, so
`supabase db reset` is safe again. #20 adds no migration.
...
- #1–#19 and #38 are merged to `main`. #20 is on `feat/invite-flow` (issue open
  until it merges). Later issues get `ready` as their dependencies close — do
  this when you finish an issue. #17 was closed by hand this session (its work
  merged in `d6ee22f` but the issue stayed open).
...
- **Auth (#17).** Sign-in is magic link + Google, PKCE, both returning through
  ...
  **web env var**, not known to Postgres: `/auth/callback` →
  ...
  up.** `/tree/*` is still 404 (issue #21); `/` and the proxy already route to
  it.
