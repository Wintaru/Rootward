/**
 * `gedcom-export` edge function (SPEC §7, issue #15) — the Deno runtime shell.
 *
 * Thin on purpose: verify the caller, build the service-role gateway, hand off
 * to {@link runExport}. All reconstruction logic — the row set back to a
 * `GedcomReadResult`, then `writeGedcom` — lives in the portable `exporter.ts`,
 * which the test suite drives directly.
 *
 * Request:  `POST { "jobId": "<uuid>" }`
 * Auth:     a moderator/admin user JWT, or the service-role key.
 * Response: `{ status, signedUrl, storagePath, sizeBytes, counts, warnings }`.
 */
